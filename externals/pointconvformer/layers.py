...
from externals.pointconvformer.util.checkpoint import CheckpointFunction
from externals.pointconvformer.layer_utils import (
    PConv,
    PCF,
    index_points,
    VI_coordinate_transform,
    Linear_BN, 
    UnaryBlock
)
...
# Custom Object PointConv Layer for PointConv Interaction Nets
class PointConvStridePE_obj_custom(nn.Module):

    def __init__(self, in_channel, out_channel, cfg, weightnet=[9, 16], res=True):
        super(PointConvStridePE_obj_custom, self).__init__()
        self.cfg = cfg
        self.in_channel = in_channel
        self.out_channel = out_channel
        self.is_residual = res

        self.drop_path = DropPath(cfg.drop_path_rate) if cfg.drop_path_rate > 0. else nn.Identity()
        # positonal encoder
        self.pe_convs = WeightNet(3, min(out_channel // 4, 32), hidden_unit=[out_channel // 4])
        last_ch = min(out_channel // 4, 32)  if self.cfg.USE_PE else 0        
    ...
    def forward(
            ...
            dense_feats,
            nei_inds,
            dense_xyz_id, # object ID assigned to point
            sparse_xyz=None,
            sparse_xyz_id=None, # object ID assigned to point
        ):
        ...
        gathered_id = index_points(dense_xyz_id, nei_inds)
        if sparse_xyz is not None:
            localized_xyz = gathered_xyz - sparse_xyz.unsqueeze(dim=2)
            id_scores = F.cosine_similarity(sparse_xyz_id.unsqueeze(dim=2), gathered_id, dim=-1)
        else:
            localized_xyz = gathered_xyz - dense_xyz.unsqueeze(dim=2)
            id_scores = F.cosine_similarity(dense_xyz_id.unsqueeze(dim=2), gathered_id, dim=-1)
        # id_scores = torch.ones_like(id_scores)
        ...
        conv_ignore = self._get_unrelevant_input_mask(localized_xyz, dist)
        id_scores = id_scores * conv_ignore.squeeze(-1)
        weights = self.weightnet(weightNetInput)
        weights = weights * id_scores.unsqueeze(-1)
        conv_norm_new = torch.sum(id_scores, dim=-1, keepdims=True) + 1.0

        new_feat = torch.matmul(input=new_feat.permute(0, 1, 3, 2), other=weights).view(B, M, -1)
        new_feat = torch.div(new_feat, conv_norm_new)
        new_feat = self.linear(new_feat)
        new_feat = self.leaky_relu(new_feat)
        ...
        if self.is_residual:
            ...
            new_feat = self.leaky_relu(self.drop_path(new_feat))

        return new_feat, weightNetInput
    
    def _get_unrelevant_input_mask(self, localized_xyz, threshold):
        
        is_close = torch.squeeze(torch.sum(localized_xyz**2, dim=3)**0.5 < threshold, dim=0)
        num_close_points = torch.sum(is_close, dim=1)
        num_close_points = torch.unsqueeze(torch.unsqueeze(num_close_points, dim=0), dim=2)
        is_close = torch.unsqueeze(torch.unsqueeze(is_close, dim=0), dim=3)
        is_close = is_close.to(torch.float)

        return is_close


# Custom Object PointConv Layer (with upsampling) for PointConv Interaction Nets
class PointConvTransposePE_obj_custom(nn.Module):

    def __init__(self, in_channel, out_channel, cfg, weightnet=[9, 16], mlp2=None):
        super(PointConvTransposePE_obj_custom, self).__init__()
        self.cfg = cfg
        self.in_channel = in_channel
        self.out_channel = out_channel
        ...
        self.drop_path = DropPath(cfg.drop_path_rate) if cfg.drop_path_rate > 0. else nn.Identity()
        # positonal encoder
        self.pe_convs = WeightNet(3, min(out_channel, 32), hidden_unit=[out_channel])
        last_ch = min(out_channel, 32)
        last_ch = 0
        ...
        self.weightnet = WeightNet(weightnet[0], weightnet[1])
        if cfg.BATCH_NORM:
            self.linear = Linear_BN((out_channel + last_ch) * weightnet[-1], out_channel, bn_ver='1d')
        else:
            self.linear = nn.Linear((out_channel + last_ch) * weightnet[-1], out_channel)
    ...
    def forward(
            ...
            sparse_feats,
            nei_inds,
            sparse_xyz_id, # object ID assigned to point
            dense_xyz,
            dense_xyz_id, # object ID assigned to point
            dense_feats=None,
        ):
        ...
        gathered_id = index_points(sparse_xyz_id, nei_inds)
        localized_xyz = gathered_xyz - dense_xyz.unsqueeze(dim=2)
        id_scores = F.cosine_similarity(dense_xyz_id.unsqueeze(dim=2), gathered_id, dim=-1)
        # id_scores = torch.ones_like(id_scores)
        ...
        # Second upscaling mlp
        # new_feat = self.unary2(new_feat)
        if dense_xyz is not None:
            shortcut = torch.max(index_points(sparse_feats, nei_inds), dim=2)[0]
        else:
            shortcut = sparse_feats

        shortcut = self.unary_shortcut(shortcut)
        new_feat = self.leaky_relu(self.drop_path(new_feat) + shortcut)

        for conv in self.mlp2_convs:
            dense_feats = conv(dense_feats)
        
        new_feat = F.leaky_relu(new_feat + dense_feats)
...
# Custom Relational PointConv Layer for PointConv Interaction Nets
# NOTE: can't handle the case when sparse_xyz != dense_xyz in this implementation
class PointConvStridePE_rel_custom(nn.Module):

    def __init__(self, in_channel, out_channel, cfg, weightnet=[9, 16]):
        super(PointConvStridePE_rel_custom, self).__init__()
        self.cfg = cfg
        self.in_channel = in_channel
        self.out_channel = out_channel
    ...
    def forward(
        ...
        B, N, _ = dense_xyz.shape
        if sparse_xyz is not None:
            raise NotImplementedError("sparse_xyz should be None for relational pointconv")
            _, M, _ = sparse_xyz.shape
        else:
            M = N
        _, _, K = nei_inds.shape
        ...
        if sparse_xyz is not None:
            localized_xyz = gathered_xyz - sparse_xyz.unsqueeze(dim=2)
            id_scores = 1.0 - F.cosine_similarity(sparse_xyz_id.unsqueeze(dim=2), gathered_id, dim=-1)
        else:
            localized_xyz = gathered_xyz - dense_xyz.unsqueeze(dim=2)
            id_scores = 1.0 - F.cosine_similarity(dense_xyz_id.unsqueeze(dim=2), gathered_id, dim=-1)
        # id_scores = torch.ones_like(id_scores) # for non-object centric experiments
        ...
        mask, conv_ignore = self._get_unrelevant_input_mask(localized_xyz, dist)
        ...
        gating = mask.to(torch.float).view(1, mask.shape[0], 1)
        inv_gating = (~mask).to(torch.float).view(1, mask.shape[0], 1)
        new_feat = gating*self.leaky_relu(new_feat + shortcut) + inv_gating*shortcut 
    ...
    def _get_unrelevant_input_mask(self, localized_xyz, threshold):
        ...
        is_close = torch.squeeze(torch.sum(localized_xyz**2, dim=3)**0.5 < threshold, dim=0)
        num_close_points = torch.sum(is_close, dim=1)
        mask_close_points = num_close_points > 1
        num_close_points = torch.unsqueeze(torch.unsqueeze(num_close_points, dim=0), dim=2)
        is_close = torch.unsqueeze(torch.unsqueeze(is_close, dim=0), dim=3)
        is_close = is_close.to(torch.float)

        return mask_close_points, is_close
